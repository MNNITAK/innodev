...
const generateStateData = (orchestrationData) => {
  // Default state data with preserved names
  ...
  // If no orchestration data, return default
  if (!orchestrationData?.geographicDistribution) {
    return defaultStateData;
  }

  // Update with orchestration data while preserving names
  const updatedStateData = { ...defaultStateData };
  
  orchestrationData.geographicDistribution.forEach(geoData => {
    // Find matching state code by comparing names
    const stateCode = Object.keys(defaultStateData).find(code => {
      const stateName = defaultStateData[code].name.toLowerCase();
      const geoName = geoData.state.toLowerCase();
      
      // Match by substring or exact match
      return stateName.includes(geoName) || 
             geoName.includes(stateName) ||
             stateName.includes(geoName.substring(0, 4)) ||
             geoName.includes(stateName.substring(0, 4));
    });
    
    if (stateCode) {
      updatedStateData[stateCode] = {
        ...updatedStateData[stateCode],
        support: Math.round(geoData.supportPercentage) // Use actual support percentage from orchestration
      };
    }
  });
...
// --- Helpers ---
const getNormalizedValue = (stateCode, stateData) => {
  const data = stateData[stateCode];
  return data ? data.support / 100 : 0;
};

const getStateCodeByName = (geoName, stateData) => {
  const nameMap = {
...
function IndiaMap() {
  ...
  // Generate state data from orchestration results
  useEffect(() => {
    const newStateData = generateStateData(dashboardData);
  ...
  const style = (feature) => {
    ...
    const stateCode = getStateCodeByName(stateName, stateData);
    const value = stateCode ? getNormalizedValue(stateCode, stateData) : 0;
