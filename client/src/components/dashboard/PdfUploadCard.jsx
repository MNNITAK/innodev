...
export const usePdfUploadContext = () => {
  ...
  if (!context) {
    throw new Error('usePdfUploadContext must be used within a PdfUploadProvider');
  }
  return context;
};

// Provider component
export const PdfUploadProvider = ({ children }) => {
...
function PdfUploadCard({ onRun, isRunning, onUploadComplete }) {
  const { 
    ...
    handleUpload: contextHandleUpload 
  } = usePdfUploadContext();

  const handleFileChangeWrapper = (e) => {
    contextHandleFileChange(e);
  };

  const handleUploadWrapper = async () => {
    try {
      await contextHandleUpload();
